...
void graph_add_edge(Graph *g, const char *from_addr, const char *to_addr,
                    double amount, int64_t timestamp_utc, const char *tx_hash,
                    int chain_id);
void graph_free(Graph *g);

TraceResult bounded_bfs_to_exchange(Graph *g, const char *start_addr,
                                    const char **exchange_addrs,
                                    int exchange_addr_count, int max_hops,
                                    int64_t max_time_window_seconds,
                                    double min_amount_threshold);

PeelingSignal detect_peeling_chain(Graph *g, const char *candidate_addr,
                                   double fan_out_ratio_threshold);
